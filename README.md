...
The same GitHub Pages deployment now supports a phone-optimized layout.

- Laptop/default: `https://YOUR-USERNAME.github.io/YOUR-REPO/`
- PowerPoint: `https://YOUR-USERNAME.github.io/YOUR-REPO/?mode=presentation`
- Mobile (forced): `https://YOUR-USERNAME.github.io/YOUR-REPO/?mode=mobile`

On screens up to 700px wide, mobile mode turns on automatically unless `?mode=presentation` is explicitly used. The mobile layout keeps the same workflow and data but reorganizes the experience for touch: workflow steps become a horizontal progress strip, the spreadsheet is swipeable, Incoming/Outgoing Connections move directly below the sheet, touch targets are larger, and Previous/Next controls stay fixed at the bottom of the screen.


## Automatic desktop / mobile switching

The default URL now selects the layout automatically from the browser viewport. At 700px wide or below it uses the mobile layout; above 700px it uses the desktop layout. The page also responds to browser resizing and phone orientation changes.

A header toggle is included for testing and manual overrides:

- **Auto** — recommended; chooses Desktop or Mobile automatically.
- **Desktop** — forces the desktop layout and updates the URL to `?mode=desktop`.
- **Mobile** — forces the mobile layout and updates the URL to `?mode=mobile`.
- **PowerPoint** — remains available directly through `?mode=presentation`; presentation mode takes priority over responsive switching.

The toggle changes the URL with `history.replaceState`, so it does not reload the demo or reset workflow progress.
