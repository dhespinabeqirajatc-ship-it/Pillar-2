...
2. Transitional Safe Harbour — review the pre-populated PASS/FAIL logic. Netherlands and Germany are both shown as FAILED in the displayed Safe Harbour result, and Germany’s De Minimis Test is FAILED.
3. Jurisdictional Elections — interact with annual/five-year election dropdowns and share the selections to WData.
4. Entity Pack — available only for the failed Netherlands path. Alpha BV and Delta BV are shown together (no simulated entity-switch control); enter additions/reductions and see calculated totals.
...
A persistent sidebar shows workflow progress. The Advance Tax Compliance logo is included in the sidebar.
...
- Safe Harbour now ships with the demo scenario already populated: Netherlands and Germany both show FAILED in the displayed Safe Harbour result.
...
- Germany - CE now displays **FAILED** in the overall Safe Harbour result, matching the Netherlands result cell, and its **De Minimis Test** remains **FAILED**.
...
- Removed the top status badge, demo notice banner, instruction banner, scenario strip, and sidebar data-flow card from the visible demo.
## Guide behavior

The step guides are available from the **? Guide** button in the top-right corner. They do not open automatically when the demo starts, restarts, or advances to the next step.
