...
// Default behavior is automatic: mobile at <= 700px and desktop above it.
// The header toggle can temporarily force Desktop or Mobile for testing.
// URL overrides are also supported: ?mode=mobile, ?mode=desktop, ?mode=presentation.
(function initResponsiveMode(){
  const params = new URLSearchParams(window.location.search);
  const urlMode = params.get("mode");
  if (urlMode === "presentation") return;

  const media = window.matchMedia("(max-width: 700px)");
  let selectedMode = urlMode === "mobile" || urlMode === "desktop" ? urlMode : "auto";

  function effectiveMode(){
    if (selectedMode === "mobile") return "mobile";
    if (selectedMode === "desktop") return "desktop";
    return media.matches ? "mobile" : "desktop";
  }

  function updateToggle(){
    document.querySelectorAll("[data-view-mode]").forEach((button) => {
      const active = button.dataset.viewMode === selectedMode;
  ...
  function applyResponsiveMode(){
    const mobile = effectiveMode() === "mobile";
  ...
  document.querySelectorAll("[data-view-mode]").forEach((button) => {
    button.addEventListener("click", () => {
      selectedMode = button.dataset.viewMode;
      const nextUrl = new URL(window.location.href);
      if (selectedMode === "auto") nextUrl.searchParams.delete("mode");
      else nextUrl.searchParams.set("mode", selectedMode);
  ...
  media.addEventListener?.("change", () => {
    if (selectedMode === "auto") applyResponsiveMode();
  });
  window.addEventListener("resize", () => {
    if (selectedMode === "auto") applyResponsiveMode();
  });
  window.addEventListener("orientationchange", () => {
    if (selectedMode === "auto") window.setTimeout(applyResponsiveMode, 50);
  });
