...
const stepMeta = {
  ...
  safe: {
    ...
    instructionCopy: "The demo already provides the financial amounts. Netherlands and Germany are shown as FAILED in the displayed Safe Harbour result. Germany’s De Minimis Test is also FAILED. Scroll right to inspect the tests, then share the outgoing results.",
...
function safeTable() {
  ...
  const jurisdictions = [
    ...
    {j: "Germany - CE -", result: "FAILED", rev: 8500000, profit: 4200000, tax: 840000, etr: "20.00%", employee: 210000, sbie: 125000, tests: ["Fail", "Pass", "Fail", "Fail"], choice: "—"},
...
function renderToolbar() {
  ...
  if (state.current === "entity") {
    ...
    toolbar.innerHTML = `<strong>Demo scenario:</strong> Netherlands and Germany are both shown as <span class="safe-status-copy fail-copy">FAILED</span>. Germany’s <strong>De Minimis Test</strong> is also <span class="safe-status-copy fail-copy">FAILED</span>.`;
...
function renderNotice() {
  ...
  if (!notice) return;
  // Show the Workiva/demo disclaimer only at the start of the flow.
  notice.classList.toggle("hidden", state.current !== "entity");
}
...
function showStep(step) {
  ...
  if ($("#instruction-number")) $("#instruction-number").textContent = i + 1;
  if ($("#instruction-title")) $("#instruction-title").textContent = m.instructionTitle;
  if ($("#instruction-copy")) $("#instruction-copy").textContent = m.instructionCopy;
  ...
  if ($("#step-status")) $("#step-status").textContent = state.shared.has(step) ? "Shared to WData" : (step === "gir" ? "Ready" : "Not shared");
  if ($("#step-status")) $("#step-status").classList.toggle("shared", state.shared.has(step) || step === "gir");
...
async function shareCurrent() {
  ...
  unlockNext();
  renderConnections();
  if ($("#step-status")) $("#step-status").textContent = "Shared to WData";
  if ($("#step-status")) $("#step-status").classList.add("shared");
...
function exportDataset() {
  ...
  return {
    reportingYear: 2024,
    safeHarbour: {Netherlands: "FAIL", Germany: "FAIL", GermanyDeMinimis: "FAIL"},
...
function exportRows() {
  ...
  return [
    ...
    ["Germany Safe Harbour overall", "FAIL"],
...
function openExport() {
  ...
  if ($("#step-status")) $("#step-status").textContent = "Dataset ready";
  renderDataFlow();
}
