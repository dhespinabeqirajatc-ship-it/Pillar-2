...
function entityPackTable() {
  ...
  const makePackRow = (rowNum, cells, cls="") => {
    const normalized = cols.map((_, i) => cells[i] || blank());
    return `<tr class="${cls}"><td class="rownum">${rowNum}</td>${normalized.map(c => `<td class="${c.cls || ""}" ${c.attrs || ""}>${c.html ?? ""}</td>`).join("")}</tr>`;
  };

  const r = [];
  r.push(`<thead><tr class="letters"><th class="rownum"></th>${cols.map(c => `<th>${c}</th>`).join("")}</tr></thead><tbody>`);
  r.push(makePackRow(1, []));
  r.push(makePackRow(2, [blank(),blank(),blank(),blank(),blank(),blank(),blank(),blank(),cell('<strong>("+/- = profit and -/- = loss*")</strong>','pack-note','colspan="4"'),blank(),blank(),blank(),blank(),blank()]));
  ...
  r.push(makePackRow(8, [blank(),blank(),cell('3.1.1','pack-ref'),cell('Profit after tax according to Group reporting','pack-desc'),blank(),blank(),blank(),cell('','pack-auto-cell'),cell('','pack-auto-cell'),cell('<input class="pack-input" data-pack-field="base" value="1,215,000" aria-label="Profit after tax addition">','pack-manual-cell'),cell('','pack-manual-cell'),blank(),cell('<strong>1,215,000</strong>','pack-total-cell'),cell('','pack-check'),cell('','pack-check'),blank(),blank()]));
  ...
  r.push(makePackRow(11, [blank(),blank(),cell('3.1.1','pack-ref'),cell('Include / exclude profits adjustment related to one jurisdiction but accounted in another jurisdiction','pack-desc'),blank(),blank(),blank(),cell('','pack-auto-cell'),cell('','pack-auto-cell'),cell('<input class="pack-input" data-pack-field="adj1plus" value="" aria-label="Adjustment addition row 11">','pack-manual-cell'),cell('<input class="pack-input" data-pack-field="adj1minus" value="" aria-label="Adjustment reduction row 11">','pack-manual-cell'),blank(),cell('—','pack-total-cell'),cell('','pack-check'),cell('','pack-check'),blank(),blank()]));
  r.push(makePackRow(12, [blank(),blank(),cell('3.1.1','pack-ref'),cell('Consolidation adjustment if in reported net income already consolidation entries have been recorded','pack-desc'),blank(),blank(),blank(),cell('','pack-auto-cell'),cell('','pack-auto-cell'),cell('<input class="pack-input" data-pack-field="adj2plus" value="" aria-label="Adjustment addition row 12">','pack-manual-cell'),cell('<input class="pack-input" data-pack-field="adj2minus" value="" aria-label="Adjustment reduction row 12">','pack-manual-cell'),blank(),cell('—','pack-total-cell'),cell('','pack-check'),cell('','pack-check'),blank(),blank()]));
  r.push(makePackRow(13, [blank(),blank(),cell('3.1.1','pack-ref'),cell('Other adjustments to profit after tax to come to FANI, as described in Pillar 2','pack-desc'),blank(),blank(),blank(),cell('','pack-auto-cell'),cell('','pack-auto-cell'),cell('<input class="pack-input" data-pack-field="adj3plus" value="" aria-label="Adjustment addition row 13">','pack-manual-cell'),cell('<input class="pack-input" data-pack-field="adj3minus" value="" aria-label="Adjustment reduction row 13">','pack-manual-cell'),blank(),cell('—','pack-total-cell'),cell('','pack-check'),cell('','pack-check'),blank(),blank()]));
  r.push(makePackRow(14, []));
  r.push(makePackRow(15, [blank(),blank(),cell('3.4 & 3.5','pack-ref'),cell('<strong>Allocations</strong><br>Allocation of GloBE income/loss related to Permanent establishments and flow through entities (fill in sheet 3a)','pack-desc'),blank(),blank(),blank(),cell('','pack-auto-cell'),cell('','pack-auto-cell'),cell('—','pack-auto-cell'),cell('—','pack-auto-cell'),blank(),cell('—','pack-total-cell'),cell('','pack-check'),cell('','pack-check'),blank(),blank()]));
  r.push(makePackRow(16, []));
  r.push(makePackRow(17, []));
  r.push(makePackRow(18, [blank(),blank(),cell('3.1.1','pack-ref'),cell('<strong>FANI amount after allocations in Articles 3.4 & 3.5</strong>','pack-desc'),blank(),blank(),blank(),cell('—','pack-total-line'),cell('—','pack-total-line'),cell('<strong>1,215,000</strong>','pack-total-line'),cell('—','pack-total-line'),blank(),cell('<strong>1,215,000</strong>','pack-total-line'),cell('','pack-check'),cell('','pack-check'),blank(),blank()]));
  ...
  r.push(makePackRow(21, [blank(),blank(),blank(),blank(),blank(),blank(),blank(),cell('Additions','pack-dark-head'),cell('Reductions','pack-dark-head'),cell('Additions','pack-dark-head'),cell('Reductions','pack-dark-head'),blank(),cell('Net total amounts','pack-dark-head'),blank(),blank(),blank(),blank()]));
  r.push(makePackRow(22, []));
  r.push(makePackRow(23, [blank(),blank(),cell('3.2.4.1.a 3.a','pack-ref'),cell('Net Tax Expense<br><span class="pack-subtext">Exclude Net taxes expense (as defined by 10.1)</span>','pack-desc'),blank(),blank(),blank(),cell('','pack-auto-cell'),cell('','pack-auto-cell'),cell('<input class="pack-input" data-pack-field="taxplus" value="35,000" aria-label="Net tax expense addition">','pack-manual-cell'),cell('','pack-manual-cell'),blank(),cell('<strong>35,000</strong>','pack-total-cell'),cell('','pack-check'),cell('','pack-check'),blank(),blank()]));
  r.push(`</tbody>`);
  return r.join('');
}
...
function renderScenario() {
  ...
  if (state.current === "safe") {
    ...
    strip.innerHTML = `<strong>Why this step is active:</strong> Netherlands failed Safe Harbour, so the detailed Entity Pack is required. The GloBE Income or Loss sheet shows how the earlier elections feed into the entity-level calculation.`;
    strip.classList.remove("hidden");
  } else {
...
function updateStep4BottomScrollbar() {
  ...
  const spacer = document.querySelector('#sheet-bottom-scroll-spacer');
  if (!sheetArea || !sheetScroll || !bottomScroll || !spacer) return;

  const isStep4 = state.current === 'entitypack';
  sheetArea.classList.toggle('step4-has-bottom-scroll', isStep4);
  bottomScroll.classList.toggle('hidden', !isStep4);
  ...
  // Match the draggable bar to the real width of the recreated HTML sheet.
  requestAnimationFrame(() => {
    spacer.style.width = `${Math.max(sheetScroll.scrollWidth, sheetScroll.clientWidth + 1)}px`;
...
window.addEventListener("resize", updateStep4BottomScrollbar);
